...
function PatientCard({ patient }) {
  ...
  const form = useForm({
    ...
    validationRules: {
      reasonForVisit: (value) => value.trim().length > 0,
      disease: (value) => value.trim().length > 0,
      treatment: (value) => value.trim().length > 0,
    },
  });
  useEffect(() => {
      ...
      <div className="flex flex-col gap-2 rounded-lg bg-white p-4 drop-shadow-2xl">
        ...
        >
          <Textarea
            ...
            error={form.errors.reasonForVisit}
          />
          <Textarea
            withAsterisk
            required
            ...
            icon={<FaNotesMedical />}
            variant="filled"
            {...form.getInputProps("diseaseDiagnosed")}
            placeholder="patient diagnosed with ?"
            error={form.errors.diseaseDiagnosed}
          />
          <Textarea
            autosize
            withAsterisk
            ...
            placeholder="prescribe treatment to patient"
